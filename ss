...
-- ==========================================
-- Global GUI Search System
-- ระบบค้นหาทั่วทั้ง GUI
-- ==========================================

-- เพิ่มใน library:Evil() function หลัง Welcome.Text
...
SearchContainer.Position = UDim2.new(0, 400, 0, 10)
...
SearchResults.Position = UDim2.new(0, 400, 0, 45)
SearchResults.Size = UDim2.new(0, 150, 0, 0)
SearchResults.Visible = false
SearchResults.ZIndex = 25
...
function AddToSearchDatabase(category, itemName, itemObject, callback)
...
local function PerformSearch(query)
    ...
    for _, item in ipairs(SearchDatabase) do
        ...
        if itemName:find(query, 1, true) then
            ...
            ResultButton.Size = UDim2.new(1, -5, 0, 30)
            ResultButton.ZIndex = 26
            ...
            CategoryLabel.Position = UDim2.new(0, 5, 0, 2)
            CategoryLabel.Size = UDim2.new(1, -10, 0, 12)
            CategoryLabel.ZIndex = 27
            ...
            NameLabel.Position = UDim2.new(0, 5, 0, 14)
    ...
    if resultsFound > 0 then
        local maxHeight = math.min(resultsFound * 35, 200)
        SearchResults.Size = UDim2.new(0, 150, 0, maxHeight)
        SearchResults.Visible = true
        SearchResults.CanvasSize = UDim2.new(0, 0, 0, resultsFound * 35)
    else
        SearchResults.Visible = false
    end
end
...
SearchBox.FocusLost:Connect(function()
    wait(0.2) -- Delay to allow clicking results
    if not SearchResults:IsMouseOver() then
        SearchResults.Visible = false
    end
end)
...
-- ==========================================
-- แก้ไขใน functionitem functions เพื่อ register อัตโนมัติ

-- ทำแบบเดียวกันกับ AddButton, AddSlider, AddSearchableDropdown, ฯลฯ
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	Main:TweenSize(UDim2.new(0,555,0,375),"Out","Quad",0.5,false) --0,585,0,475 PC 
	...
	function library:CreateTab(options)
		...
		-- ==========================================
		-- Enhanced Dropdown with Search Feature
		-- เพิ่มฟีเจอร์ Search ใน Dropdown
		-- ==========================================

		function functionitem:AddSearchableDropdown(options)
			local text = options.Name
			local Flag = options.Flag
		...
		function functionitem:AddSearchableMultiDropdown(options)
			local text = options.Name
			local Flag = options.Flag
			local default = options.Value or {}  -- รองรับ table ของ default values
			local list = options.List
			local callback = options.Callback
			
			local Dropfunc = {}
			local DropTable = {}  -- เก็บ items ที่เลือกไว้
			...
			-- TextLabel ที่แสดงผลการเลือก (ปรับให้ชิดซ้ายและตัดข้อความยาว)
			Text_2.Name = "Text"
			Text_2.Parent = MainDropDown_2
			Text_2.BackgroundTransparency = 1.000
			Text_2.Position = UDim2.new(0, 12, 0, 0)
			Text_2.Size = UDim2.new(1, -50, 1, 0)  -- เว้นพื้นที่ให้ลูกศร
			Text_2.ZIndex = 16
			Text_2.Font = Enum.Font.GothamBold
			Text_2.TextSize = 13.000
			Text_2.TextXAlignment = Enum.TextXAlignment.Left
			Text_2.TextTruncate = Enum.TextTruncate.AtEnd  -- เพิ่ม ... ถ้ายาวเกิน
			Text_2.Text = "Select items..."
			...
			Scroll_Items.BorderSizePixel = 0
			Scroll_Items.Position = UDim2.new(0, 0, 0, 75)
			Scroll_Items.Size = UDim2.new(1, 0, 0, 224)  -- ความสูงพอให้เลื่อนได้
			Scroll_Items.CanvasSize = UDim2.new(0, 0, 0, 0)
			Scroll_Items.ScrollBarThickness = 2
			Scroll_Items.Visible = false
			...
			-- Toggle Panel
			function Dropfunc:TogglePanel(state)
				TweenService:Create(MainDropDown, TweenInfo.new(.2, Enum.EasingStyle.Quad),
					{Size = state and UDim2.new(0, 265, 0, 340) or UDim2.new(0, 265, 0, 35)}):Play()
				TweenService:Create(ImageButton, TweenInfo.new(.2, Enum.EasingStyle.Quad),
					{Rotation = state and 180 or 0}):Play()
			...
			function Dropfunc:FilterItems(query)
				...
				for _, item in pairs(Scroll_Items:GetChildren()) do
					if item:IsA("TextButton") then
						local itemText = item.Text:lower()
						item.Visible = query == "" or itemText:find(query, 1, true) ~= nil
					end
				end
			end
			...
			-- Clear
			function Dropfunc:Clear()
				for _, v in pairs(Scroll_Items:GetChildren()) do
			...
			function Dropfunc:Add(Text)
				...
				-- ไฮไลต์ถ้าถูกเลือกอยู่แล้ว
				if table.find(DropTable, Text) then
					btn.TextColor3 = Color3.fromRGB(0, 200, 200)
				end
				
				btn.MouseEnter:Connect(function()
					TweenService:Create(stroke, TweenInfo.new(0.4), {Transparency = 0.10}):Play()
				end)
				...
				btn.MouseButton1Click:Connect(function()
					...
					if not found then
						...
						TweenService:Create(btn, TweenInfo.new(.2), {TextColor3 = Color3.fromRGB(0, 200, 200)}):Play()
					else
						table.remove(DropTable, found)
						TweenService:Create(btn, TweenInfo.new(.2), {TextColor3 = Color3.fromRGB(255, 255, 255)}):Play()
					end
					
					UpdateDisplayText()
					callback(DropTable)  -- ส่ง table ทั้งหมดกลับไป
				end)
			end
			...
			-- ตั้งค่าข้อความเริ่มต้นให้ถูกต้อง
			UpdateDisplayText()
	...
	-- ==========================================
-- 🔍 COMPLETE GLOBAL GUI SEARCH SYSTEM (เวอร์ชันสมบูรณ์)
-- ==========================================

-- วางโค้ดนี้ใน library:Evil() หลัง Title2 เหมือนเดิม

local SearchSystem = {}
SearchSystem.Database = {}
SearchSystem.CurrentTab = nil
...
function SearchSystem:PerformSearch(query)
    ...
    searchDebounce = true
    task.wait(0.15)  -- Debounce เพื่อป้องกัน lag
    ...
    for _, item in ipairs(self.Database) do
        ...
        local sectionName = item.Section and item.Section:lower() or ""
        
        if itemName:find(query, 1, true) or tabName:find(query, 1, true) or sectionName:find(query, 1, true) then
            resultsFound = resultsFound + 1
            ...
            ResultButton.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
            ResultButton.BorderSizePixel = 0
            ResultButton.Size = UDim2.new(1, -10, 0, 48)
            ...
            CategoryIcon.Position = UDim2.new(0, 5, 0, 4)
            CategoryIcon.Size = UDim2.new(0, 20, 0, 14)
            ...
            PathLabel.Position = UDim2.new(0, 28, 0, 24)
            PathLabel.Size = UDim2.new(1, -35, 0, 14)
            ...
            ResultButton.MouseLeave:Connect(function()
                TweenService:Create(ResultButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(30, 30, 30)}):Play()
            end)
    ...
    if resultsFound > 0 then
        ...
        NoResults.Size = UDim2.new(1, -10, 0, 50)
...
function SearchSystem:NavigateToItem(item)
    local targetTabName = item.Tab
    
    -- ค้นหาและคลิก Tab ที่ตรงกัน
    for _, tab in pairs(TabContainer:GetChildren()) do
        if tab:IsA("TextButton") and tab:FindFirstChild("TextLabel") and tab.TextLabel.Text == targetTabName then
            -- Simulate click บน tab นั้น (เรียกฟังก์ชันเดิมของ library ถ้ามี หรือจำลอง)
            -- สมมุติว่า library มีฟังก์ชันเปลี่ยน tab อยู่แล้ว เราจำลองเหมือนการคลิก
            
            -- รีเซ็ต tab ทั้งหมดก่อน
            for _, t in pairs(TabContainer:GetChildren()) do
                if t:IsA("TextButton") and t:FindFirstChild("TextLabel") then
                    TweenService:Create(t.TextLabel, TweenInfo.new(0.3), {TextTransparency = 0.4}):Play()
                    if t:FindFirstChild("ImageLabel") then
                        TweenService:Create(t.ImageLabel, TweenInfo.new(0.3), {ImageTransparency = 0.4}):Play()
                    end
                    if t:FindFirstChild("FrameTab") then
                        t.FrameTab.Visible = false
                    end
                end
            end
            
            -- เปิด tab ที่ต้องการ
            TweenService:Create(tab.TextLabel, TweenInfo.new(0.3), {TextTransparency = 0}):Play()
            if tab:FindFirstChild("ImageLabel") then
                TweenService:Create(tab.ImageLabel, TweenInfo.new(0.3), {ImageTransparency = 0}):Play()
            end
            if tab:FindFirstChild("FrameTab") then
                tab.FrameTab.Visible = true
            end
            
            -- ซ่อนทุก Page ก่อน
            for _, page in pairs(Bottom:GetChildren()) do
                if page.Name == "Page" or page:IsA("Frame") then
                    page.Visible = false
                end
            end
            
            -- แสดง Page ที่ตรงกับ Tab
            local pageName = "Page" .. targetTabName:gsub(" ", "")
            for _, page in pairs(Bottom:GetChildren()) do
                if page.Name == pageName or (page:FindFirstChild("Name") and page.Name.Text == targetTabName) then
                    page.Visible = true
                    break
                end
            end
            ...
            if item.Object and item.Object.Parent then
                ...
                for i = 1, 3 do
                    task.delay(i * 0.3, function()
                        TweenService:Create(obj, TweenInfo.new(0.2), {BackgroundColor3 = _G.Color}):Play()
                        task.wait(0.2)
                        TweenService:Create(obj, TweenInfo.new(0.2), {BackgroundColor3 = originalColor}):Play()
                    end)
                end
                ...
                if scrollFrame then
                    local targetY = obj.AbsolutePosition.Y - scrollFrame.AbsolutePosition.Y - 50
...
SearchBox.Focused:Connect(function()
    TweenService:Create(SearchStroke, TweenInfo.new(0.3), {Transparency = 0, Thickness = 2}):Play()
    TweenService:Create(SearchIcon, TweenInfo.new(0.3), {ImageColor3 = Color3.fromRGB(255, 255, 255)}):Play()
end)

SearchBox.FocusLost:Connect(function()
    TweenService:Create(SearchStroke, TweenInfo.new(0.3), {Transparency = 0.5, Thickness = 1.5}):Play()
    TweenService:Create(SearchIcon, TweenInfo.new(0.3), {ImageColor3 = _G.Color}):Play()
...
-- ==========================================
-- KEYBOARD SHORTCUT (Ctrl+F & Escape)
-- ==========================================
local UserInputService = game:GetService("UserInputService")
UserInputService.InputBegan:Connect(function(input, gameProcessed)
...
-- Override CreateTab และ CreateSection เพื่อเก็บชื่อปัจจุบัน
local oldCreateTab = tabs.CreateTab
tabs.CreateTab = function(self, options)
    CurrentTabName = options.Name or "Unknown Tab"
    return oldCreateTab(self, options)
end

local oldCreateSection = sections.CreateSection
sections.CreateSection = function(self, options)
    CurrentSectionName = options.Name or "Unknown Section"
    return oldCreateSection(self, options)
end

-- Wrapper สำหรับทุก Add function
local function WrapAddFunction(original, category)
    return function(self, options)
        local result = original(self, options)
        
        local cleanName = (options.Name or options.Text or "Unknown"):gsub("[\n\r].*", ""):match("^%s*(.-)%s*$")
        
        local uiObject = nil
        if type(result) == "table" and result.Object then
            uiObject = result.Object
        elseif result:IsA("GuiObject") then
            uiObject = result
        ...
        if uiObject then
            SearchSystem:AddToDatabase(category, cleanName, CurrentTabName, CurrentSectionName, uiObject)
        end
...
-- ลงทะเบียนทุก component
if functionitem then
    functionitem.AddToggle               = WrapAddFunction(functionitem.AddToggle, "Toggle")
    functionitem.AddButton               = WrapAddFunction(functionitem.AddButton, "Button")
    functionitem.AddSlider               = WrapAddFunction(functionitem.AddSlider, "Slider")
    functionitem.AddSearchableDropdown   = WrapAddFunction(functionitem.AddSearchableDropdown, "Dropdown")
    functionitem.AddSearchableMultiDropdown = WrapAddFunction(functionitem.AddSearchableMultiDropdown, "MultiDropdown")
    functionitem.AddTextbox              = WrapAddFunction(functionitem.AddTextbox, "Textbox")
    functionitem.AddLabel                = WrapAddFunction(functionitem.AddLabel, "Label")
    functionitem.AddLabelX               = WrapAddFunction(functionitem.AddLabelX, "Label")
    -- เพิ่ม component อื่น ๆ ที่มีใน library ได้เลย
end
			return functionitem
		end
		return sections
