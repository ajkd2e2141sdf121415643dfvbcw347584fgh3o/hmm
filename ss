...
-- ==========================================
-- 🔍 COMPLETE GLOBAL GUI SEARCH SYSTEM
-- ==========================================

-- เพิ่มใน library:Evil() หลัง Title2
local SearchSystem = {}
...
function SearchSystem:PerformSearch(query)
    ...
    for _, child in pairs(SearchResults:GetChildren()) do
        if child:IsA("TextButton") or child:IsA("Frame") then
            if child.Name ~= "UIPadding" and not child:IsA("UIListLayout") then
                child:Destroy()
            end
        end
    end
    ...
    query = query:lower()
    local resultsFound = 0
    local maxResults = 15 -- จำกัดผลลัพธ์
...
function SearchSystem:NavigateToItem(item)
    ...
    for _, tab in pairs(TabContainer:GetChildren()) do
        if tab:IsA("TextButton") and tab.TextLabel.Text:find(item.Tab) then
            -- Simulate tab click
            for _, x in next, TabContainer:GetChildren() do
                if x.Name == "Tab" then
                    TweenService:Create(x.TextLabel, TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                        {TextColor3 = Color3.fromRGB(255, 255, 255), TextTransparency = 0.2}):Play()
                    TweenService:Create(x.ImageLabel, TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                        {ImageColor3 = Color3.fromRGB(255, 255, 255), ImageTransparency = 0.2}):Play()
            ...
            TweenService:Create(tab.TextLabel, TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                {TextColor3 = Color3.fromRGB(255, 255, 255), TextTransparency = 0}):Play()
            TweenService:Create(tab.ImageLabel, TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                {ImageColor3 = Color3.fromRGB(255, 255, 255), ImageTransparency = 0}):Play()
            tab.FrameTab.Visible = true
            TweenService:Create(tab.FrameTab, TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                {Size = UDim2.new(0, 50, 0, 1)}):Play()
            
            -- Show page
            local pageName = tab.TextLabel.Text:gsub(" ", "")
...
SearchBox.FocusLost:Connect(function()
    ...
    -- Hide results after a delay (allow clicking)
    task.wait(0.2)
...
-- AUTO-REGISTER COMPONENTS
-- ==========================================
-- แก้ไข functionitem functions ให้ register อัตโนมัติ

-- Store current tab and section context
local CurrentTabName = ""
local CurrentSectionName = ""

-- Modify CreateTab to store tab name
local originalCreateTab = tabs.CreateTab
function tabs:CreateTab(options)
    CurrentTabName = options.Name
    local result = originalCreateTab(self, options)
    return result
end

-- Modify CreateSection to store section name
local originalCreateSection = sections.CreateSection
function sections:CreateSection(options)
    CurrentSectionName = options.Name
    local result = originalCreateSection(self, options)
    return result
end

-- Wrap all Add functions
local function WrapAddFunction(originalFunc, category)
    return function(self, options)
        local result = originalFunc(self, options)
        
        -- Extract clean name (remove Thai translation)
        local cleanName = options.Name or options.Text or "Unknown"
        cleanName = cleanName:gsub("\n.*", ""):gsub("\\n.*", "")
        
        -- Find the actual UI object
        local uiObject = nil
        if type(result) == "table" then
            -- For components that return tables (Toggle, Slider, etc.)
            uiObject = result.Parent or result.Object or result
        else
            -- For direct Frame/Instance returns
            uiObject = result
        end
...
local Vectorhub = library:Evil()

	local InfoTab = Vectorhub:CreateTab({
		Name = "Information" 
	})
	...
	local R = InfoSection:AddLabel({Name = "..."})
	local P = InfoSection:AddLabel({Name = "..."})
	function UpdateInfo()
